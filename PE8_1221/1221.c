...
treenode* inorder(int in[], int post[], int instart, int inend, int poststart, int postend){
    ...
    root->left = inorder(in, post, instart, i-1, poststart, poststart+leftnum-1);
    root->right = inorder(in, post, i+1, inend, poststart+leftnum, postend-1);
...
int main(){
    ...
    treenode* root = inorder(in, post, 0, i-1, 0, j-1);
